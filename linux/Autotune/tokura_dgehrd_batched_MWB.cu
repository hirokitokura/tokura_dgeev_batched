...
)
{
	...
	//__shared__ double house[32/*MATRIX_SIZE - 1*/][32];

	extern __shared__ double shared_dynamic[];
	//extern __shared__ double house_s[];
	//__shared__ double sum_tmp_shared[32/*HOUSE_KERNEL_THREAD_JOIN_NUM*/ * 32];
	...
	for (k = 1; k < n - 1; k++)
	{
		...
		tmp = house[/*matrix_id +*/ ((k - 1) *MATRIX_NUM_PER_BLOCK)] * house[/*matrix_id +*/ ((k - 1)*MATRIX_NUM_PER_BLOCK)] + tmp_sum;
